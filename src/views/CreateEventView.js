  ...
  const createEventBtn = () => {
    return (
      <BasicBtn
        type="submit"
        label="Crear evento"
        styles={{ marginTop: "15px", justifySelf: "flex-end" }}
      />
    );
  };

  const basicTextInput = (id, label) => {
    return <TextField required fullWidth id={id} label={label} name={id} />;
  };
    ...
    <main style={{ backgroundColor: "#eeeeee", minHeight: "100vh" }}>
      ...
      <Box style={styles.formContainer}>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 5 }}>
              ...
              <Grid container direction="column" spacing={2}>
                ...
                <Grid item sx={{ mt: 2 }}>
                  <TextField
                    required
                    fullWidth
                    inputProps={{ type: "number", min: 0, step: 1 ,pattern:"[0-9]*" ,inputmode:"numeric"}}
                    id="quantity"
          ...
          <Box style={styles.row}>{basicTextInput("adress", "Dirección")}</Box>
          <Typography component="h2" style={styles.subTitle}>
            Galería
          </Typography>
